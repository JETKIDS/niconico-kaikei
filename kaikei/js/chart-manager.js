...
class ChartManager {
    ...
    calculateMonthlyBalance(year, month) {
        ...
        // 合計計算
        const sales = salesData.reduce((sum, record) => sum + (record.amount || 0), 0);
        const purchases = purchasesData.reduce((sum, record) => sum + (record.amount || 0), 0);
        const fixedCosts = fixedCostsData.reduce((sum, record) => sum + (record.amount || 0), 0);
        const variableCosts = variableCostsData.reduce((sum, record) => sum + (record.amount || 0), 0);
        const laborCosts = laborCostsData.reduce((sum, record) => sum + (record.amount || 0), 0);
        const consumptionTax = consumptionTaxData.reduce((sum, record) => sum + (record.amount || 0), 0);
        const monthlyPayments = monthlyPaymentsData.reduce((sum, record) => sum + (record.amount || 0), 0);
        const manufacturerDeposits = manufacturerDepositsData.reduce((sum, record) => sum + (record.amount || 0), 0);
